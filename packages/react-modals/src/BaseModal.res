...
module Styles = {
  ...
  let scrollContainer = style([
    height(pct(100.)),
    display(#flex),
    flexDirection(#column),
    backgroundColor(rgba(74, 74, 74, #num(0.5))),
  ])
  let scrollContent = style([
    display(#flex),
...
let make = (
  ...
  <InOutCSSTransition _in ?onExit onExited>
    {animationState => {
      let (animationWrapperClassName, animationContentClassName) = getAnimationClassNames(
        animationState,
      )
